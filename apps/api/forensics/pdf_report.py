...
def generate_forensic_pdf(
    ...
    # ── Entity Verification ──────────────────────────────────────────────────
    story.append(Paragraph("3b. Entity Verification (Investigative Signals)", _HEADING_STYLE))
    story.append(Paragraph(
        "IMPORTANT: The following are investigative signals for human judgment only, not "
        "automated fraud determinations. They provide useful leads for investigators and "
        "do not constitute legal evidence about any company or individual.",
        ParagraphStyle("caution", parent=_BODY_STYLE, textColor=colors.HexColor("#b35900")),
    ))
    story.append(Spacer(1, 0.2 * cm))
    if entity_verification:
        ev_data = [["Entity", "MCA Verify Link", "Domain Age", "Domain Flag", "Public Presence"]]
        for ev in entity_verification:
            ev_data.append([
                ev.get("entity_name", ""),
                ev.get("mca_search_url", "")[:60] + "...",
                str(ev.get("domain_age_days") or "N/A"),
                (ev.get("domain_flag") or "none")[:60],
                "Yes" if ev.get("public_presence_found") else "No",
            ])
        t = Table(ev_data, colWidths=[3.5 * cm, 4 * cm, 2 * cm, 3.5 * cm, 3 * cm])
        ...
        story.append(Paragraph("No entity verification data available for this call.", _BODY_STYLE))
    story.append(Spacer(1, 0.3 * cm))
    ...
    # ── Video Evidence ─────────────────────────────────────────────────────────
    if video_frames:
        story.append(Paragraph("8. Video-Frame Evidence (Stubbed Capture)", _HEADING_STYLE))
        story.append(Paragraph(
            "PRIVACY NOTICE: The following frames were processed for presence detection ONLY "
            "using an OpenCV Haar Cascade classifier. No facial recognition, identity matching, "
            "or database lookup was performed. The SHA-256 hash provides chain-of-custody "
            "integrity verification for each frame.",
            ParagraphStyle("privacy", parent=_BODY_STYLE, textColor=colors.HexColor("#b35900")),
        ))
        story.append(Spacer(1, 0.2 * cm))
        vf_data = [["#", "Timestamp", "SHA-256 Hash", "Face Detected", "Local Path"]]
        ...
        for i, frame in enumerate(video_frames):
            ...
            if local_path and _os.path.exists(local_path):
                try:
                    story.append(Paragraph(
                        f"<b>Frame {i + 1} Thumbnail</b> — SHA-256: {frame.get('sha256_hash', '')[:16]}...",
                        _BODY_STYLE,
                    ))
                    story.append(Image(local_path, width=8 * cm, height=6 * cm))
                    story.append(Spacer(1, 0.2 * cm))
                except Exception as _thumb_err:
                    logger.warning("Could not embed frame thumbnail: %s", _thumb_err)
