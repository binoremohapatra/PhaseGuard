"""
RawNet2 ONNX Adapter for PhaseGuard Detection
...
class RawNet2Detector(DeepfakeDetector):
    """RawNet2 ONNX model detector for real deepfake detection."""

    def __init__(self, model_path: str = "models/rawnet2/rawnet2.onnx",
                 threshold_config: ThresholdConfig = None):
        """
        Initialize RawNet2 detector.

        Args:
            model_path: Path to RawNet2 ONNX model
    ...
    def initialize(self) -> bool:
        """Initialize RawNet2 ONNX model."""
        try:
            print(f"Loading RawNet2 model from {self.model_path}...")
            start_time = time.time()

            # Check if model file exists
            import os
            if not os.path.exists(self.model_path):
                print(f"RawNet2 model not found at {self.model_path}")
                self._is_initialized = False
                return False
            ...
            print(f"RawNet2 loaded successfully:")
            ...
            print(f"Failed to initialize RawNet2: {e}")
            return False
    ...
    def predict(self, audio: np.ndarray) -> Dict:
        ...
        try:
            ...
            # RawNet2 typically expects specific input format
            ...
            print(f"Error in RawNet2 prediction: {e}")
    ...
    def get_info(self) -> Dict:
        """Get RawNet2 detector information."""
        if not self._is_initialized:
