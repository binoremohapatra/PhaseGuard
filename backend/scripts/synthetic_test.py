...
HARMONIC_FREQS = [200, 400, 600, 800, 1000, 1200]  # Hz
...
def run_test() -> bool:
    ...
    if coherent_result["pdi_score"] < incoherent_result["pdi_score"]:
        print(f"  ✓ PASS: Coherent PDI ({coherent_result['pdi_score']:.4f}) < "
              f"Incoherent PDI ({incoherent_result['pdi_score']:.4f})")
    else:
        print(f"  ✗ FAIL: Expected coherent PDI < incoherent PDI")
    ...
    if incoherent_result["is_synthetic"]:
        print(f"  ✓ PASS: Phase-randomized signal correctly flagged as SYNTHETIC")
    else:
        print(f"  ✗ FAIL: Phase-randomized signal NOT flagged as synthetic "
              f"(PDI={incoherent_result['pdi_score']:.4f}, threshold={incoherent_result['threshold_used']})")
        passed = False

    print("=" * 60)
    if passed:
        print("ALL ASSERTIONS PASSED — DSP pipeline is correctly functioning ✓")
    else:
